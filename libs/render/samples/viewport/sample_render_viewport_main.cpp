...
int main()
{
	...
	{
		auto window = std::make_unique<hamon::Window>(width, height, "sample_render_viewport OpenGL");
	...
	{
		auto window = std::make_unique<hamon::Window>(width, height, "sample_render_viewport D3D11");
	...
	{
		auto window = std::make_unique<hamon::Window>(width, height, "sample_render_viewport D3D12");
	...
	{
		auto window = std::make_unique<hamon::Window>(width, height, "sample_render_viewport Vulkan");
	...
	for (;;)
	{
		...
		if (((count / 300) % 2) == 0)
		{
			render_pass.viewport.left   = 0;
			render_pass.viewport.top    = 0;
			render_pass.viewport.width  = hw + hw * std::sin(t);
			render_pass.viewport.height = hh + hh * std::sin(t);
		}
		else
		{
			render_pass.viewport.left   = hw * std::sin(t);
			render_pass.viewport.top    = hh * std::sin(t);
			render_pass.viewport.width  = width;
			render_pass.viewport.height = height;
		}
