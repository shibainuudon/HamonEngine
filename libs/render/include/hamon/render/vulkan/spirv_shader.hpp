/**
 *	@file	spirv_shader.hpp
 *
 *	@brief	SpirvShader
 */

#ifndef HAMON_RENDER_VULKAN_SPIRV_SHADER_HPP
#define HAMON_RENDER_VULKAN_SPIRV_SHADER_HPP
...
{
public:
	explicit SpirvShader(render::Shader const& shader)
		: m_spriv(GLSLtoSPV(shader))
		, m_stage(shader.GetStage())
	{
	}

	std::vector<unsigned int> const& GetCode(void) const
	{
		return m_spriv;
	}
...
private:
	...
	static std::vector<unsigned int> GLSLtoSPV(render::Shader const& glsl_shader)
	{
...
private:
	std::vector<unsigned int>	m_spriv;
...
#endif // HAMON_RENDER_VULKAN_SPIRV_SHADER_HPP
