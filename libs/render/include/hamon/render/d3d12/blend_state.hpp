...
public:
	explicit BlendState(render::BlendState const& blend_state)
	{
		...
		for (auto& rt : m_desc.RenderTarget)
		{
			...
			rt.SrcBlend              = ToD3D12Blend(blend_state.color_src_factor);
			rt.DestBlend             = ToD3D12Blend(blend_state.color_dest_factor);
			rt.BlendOp               = ToD3D12BlendOp(blend_state.color_operation);
			rt.SrcBlendAlpha         = ToD3D12Blend(blend_state.alpha_src_factor);
			rt.DestBlendAlpha        = ToD3D12Blend(blend_state.alpha_dest_factor);
			rt.BlendOpAlpha          = ToD3D12BlendOp(blend_state.alpha_operation);

			rt.LogicOpEnable         = blend_state.logic_op_enable;
			rt.LogicOp               = ToD3D12LogiOp(blend_state.logic_operation);
			
			rt.RenderTargetWriteMask = ToD3D12WriteMask(blend_state.color_write_mask);
		}
	}
	
	::D3D12_BLEND_DESC const& Get(void) const { return m_desc; }

private:
	static ::D3D12_BLEND ToD3D12Blend(render::BlendFactor factor)
	{
		switch (factor)
		{
	...
	static ::D3D12_BLEND_OP ToD3D12BlendOp(render::BlendOperation op)
	{
		switch (op)
		{
	...
	static ::D3D12_LOGIC_OP ToD3D12LogiOp(render::LogicOperation op)
	{
		switch (op)
		{
		case render::LogicOperation::Clear:			return D3D12_LOGIC_OP_CLEAR;
		case render::LogicOperation::Set:			return D3D12_LOGIC_OP_SET;
		case render::LogicOperation::Copy:			return D3D12_LOGIC_OP_COPY;
		case render::LogicOperation::CopyInverted:	return D3D12_LOGIC_OP_COPY_INVERTED;
		case render::LogicOperation::Noop:			return D3D12_LOGIC_OP_NOOP;
		case render::LogicOperation::Invert:		return D3D12_LOGIC_OP_INVERT;
		case render::LogicOperation::And:			return D3D12_LOGIC_OP_AND;
		case render::LogicOperation::Nand:			return D3D12_LOGIC_OP_NAND;
		...
		case render::LogicOperation::OrInverted:	return D3D12_LOGIC_OP_OR_INVERTED;
		}
		return D3D12_LOGIC_OP_NOOP;
	}

	static ::UINT8 ToD3D12WriteMask(std::uint32_t mask)
	{
		::UINT8 result = 0;
